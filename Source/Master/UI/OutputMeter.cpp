...
MasterOutputMeter::MasterOutputMeter (MasterProcessor& p)
    : processor (p)
{
    startTimerHz (30);
}

MasterOutputMeter::~MasterOutputMeter()
{
    stopTimer();
}

float MasterOutputMeter::toDb (float linear) noexcept
{
    if (linear <= 1.0e-6f)
        return kMinDb;
...
void MasterOutputMeter::timerCallback()
{
...
void MasterOutputMeter::drawBar (juce::Graphics& g, juce::Rectangle<float> bounds, float db, float holdDb) const
{
    k3ch::Palette pal;
...
void MasterOutputMeter::paint (juce::Graphics& g)
{
    k3ch::Palette pal;
    ...
    const juce::String modeTag = processor.isMasterMode() ? dbText : "THRU";
    g.drawFittedText (processor.isBypassed() ? "BYPASS" : modeTag,
