...
OutputMeter::OutputMeter (PresetsProcessor& p)
    : processor (p)
{
    startTimerHz (30);
}
...
float OutputMeter::toDb (float linear) noexcept
{
    if (linear <= 1.0e-6f)
        return kMinDb;
...
void OutputMeter::drawBar (juce::Graphics& g, juce::Rectangle<float> bounds, float db, float holdDb) const
{
    k3ch::Palette pal;
